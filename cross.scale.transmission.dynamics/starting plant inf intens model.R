...
data.vec<-c() 
for(tag in unique(infection.intensity$Tag)) # loop to pull out plant inf intens of newly diseased plants
{
  if((tag %in% corrected.epi$Tag))
  {
    inf.date<-corrected.epi[which(corrected.epi$Tag==tag),"Date.First.Observed.Diseased"]
    sub.data<-infection.intensity[which(infection.intensity$Tag==tag),]
    ...
    if(inf.date>=min.date) {
      index<-which(sub.data$Date==inf.date)
      data.vec<-c(data.vec,sub.data[index,"infection.intensity"])
    }
  }
}
