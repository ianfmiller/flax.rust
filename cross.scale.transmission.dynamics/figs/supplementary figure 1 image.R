...
rgb_contrast = scales::rescale(rgb_array,to=c(.2,1))

#plot_map(rgb_contrast)

plot_3d(rgb_contrast, el_matrix, windowsize = c(1250,800), zscale = 15,
        zoom=.55, phi=32,theta=230,fov=1, background = 'white')
...
render_label(heightmap=el_matrix,text="CC",lat=points@coords[1,2],long=points@coords[1,1],extent=e,textcolor="white",linecolor = site.cols[1],z=200,zscale=15,textsize = 2,linewidth = 4)
render_label(heightmap=el_matrix,text="BT",lat=points@coords[2,2],long=points@coords[2,1],extent=e,textcolor="white",linecolor = site.cols[2],z=750,zscale=15,textsize = 2,linewidth = 4)
render_label(heightmap=el_matrix,text="GM",lat=points@coords[3,2],long=points@coords[3,1],extent=e,textcolor="white",linecolor = site.cols[3],z=2000,zscale=15,textsize = 2,linewidth = 4)
render_label(heightmap=el_matrix,text="HM",lat=points@coords[4,2],long=points@coords[4,1],extent=e,textcolor="white",linecolor = site.cols[4],z=2500,zscale=15,textsize = 2,linewidth = 4)

render_snapshot() #save as ~/Documents/GitHub/flax.rust/cross.scale.transmission.dynamics/figs/figure.1.files/B.jpg
# export at dimensions 1275x788, then crop to minimum bounding dimensions
