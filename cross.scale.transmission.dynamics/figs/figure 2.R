...
p.vals<-summary(pustule.model)$s.table[,4] #p values
...
plot(0,0,xlim=c(start.date,max(sim.dates)),ylim=c(.1,.85),type="n",xlab="date",ylab=expression('pustule area ('*mm^2*')'),cex.lab=2,axes=F)
grid()
