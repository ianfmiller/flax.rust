if(!(file.exists("~/Documents/GitHub/flax.rust/cross.scale.transmission.dynamics/summarized data/foi.data.RDS")))
{
  ...
  for(site in sites)
  {
    ...
    for(date.index in (1:length(data.dates[which(sites==site)][[1]])))
    {
      ...
      obs.time<-delta.days
      plant.model.pred.data<-data.frame("plant.inf.intens"=plant.model.new.plant.inf.intens,"dew.point.days"=new.dew.point.days,"temp.7.30.dew.point.days"=new.temp.7.30.dew.point.days,"temp.wetness.days"=new.temp.wetness.days,"tot.rain"=new.tot.rain,"pred.pustule.num.increase"=pred.pustule.num.increase)
      pred.plant.inf.intens.increase<-predict(plant.model,newdata=plant.model.pred.data,re.form=NA)
      ...
      for(index in 1:dim(sub.loc.data)[1])
      {
        ...
        if(length(which(ID.strings==ID.string))==1)
        {
          ...
          foi.data<-rbind(foi.data,data.frame("site"=site,"date"=date0,"status"=status,"status.next"=new.status,"tag"=sub.loc.data[index,"tag"],"X"=sub.loc.data[index,"X"],"Y"=sub.loc.data[index,"Y"],"x"=sub.loc.data[index,"x"],"y"=sub.loc.data[index,"y"],"first.obs.height"=sub.loc.data[index,"height.cm"],"foi"=foi,
                                              ...
                                              "gust.speed.days"=new.gust.speed.days,"temp.16.22.dew.point.days"=new.temp.16.22.dew.point.days,"temp.7.30.dew.point.days"=new.temp.7.30.dew.point.days,"temp.wetness.days"=new.temp.wetness.days,"temp.16.22.wetness.days"=new.temp.16.22.wetness.days,"temp.7.30.wetness.days"=new.temp.7.30.wetness.days,
